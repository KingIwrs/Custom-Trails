...
scoreboard players set @s trails.iterator 0
function trails:customize_trails/primary_colors/get_owned_colors/start with storage trails:temp players

$execute if data storage trails:data players[{uuid:$(uuid)}].equipped{primary_color:""} run return fail

function trails:customize_trails/primary_colors/get_equipped_color/start with storage trails:temp players
